...
class Qwen3OmniNext_Vit(HuggingFaceVit):
    module_mapping = {'thinker.audio_tower': 'audio_tower', 'thinker.visual': 'visual'}
    _vision_tower = ['audio_tower', 'visual']
    _aligner = ['audio_tower.proj1', 'audio_tower.proj2', 'visual.merger', 'visual.deepstack_merger_list']
    _generator = ['talker']

    def prepare_model(self, hf_config):
        from transformers.models.qwen3_omni_next.modeling_qwen3_omni_next import (
            Qwen3OmniNextAudioEncoder, Qwen3OmniNextThinkerForConditionalGeneration, Qwen3OmniNextVisionEncoder)
        self.model_cls = Qwen3OmniNextThinkerForConditionalGeneration
        self.audio_tower = Qwen3OmniNextAudioEncoder._from_config(hf_config.thinker_config.audio_config)
        self.visual = Qwen3OmniNextVisionEncoder._from_config(hf_config.thinker_config.vision_config)
